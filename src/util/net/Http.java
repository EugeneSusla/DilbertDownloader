...
public class Http {
	...
	public static String getResponsePart(URL url, String start, String end)
			throws IOException {
		StringBuilder result = new StringBuilder();
		...
		while ((inputLine = in.readLine()) != null) {
			if (writing) {
				...
				if (endIndex != -1) {
					result.append(inputLine.substring(0,
							endIndex + end.length()));
					writing = false;
				} else {
					result.append(inputLine).append('\n');
				...
				if (startIndex != -1) {
					...
					if (endIndex == -1) {
						...
						result.append(inputLine.substring(startIndex, endIndex
								+ end.length()));
						break;
					}
				}
	...
	public static String getResponsePart(String url, String start, String end)
			throws IOException {
		return getResponsePart(new URL(url), start, end);
	}

	public static void writeResponseToFile(URL url, File file)
			throws IOException {
		URLConnection urlConnection = url.openConnection();
	...
	public static void writeResponseToFile(String url, String fileName)
			throws IOException {
